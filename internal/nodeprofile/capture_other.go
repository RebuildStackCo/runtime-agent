...
// is always unavailable, reported as a graceful refusal.
func startCapture(_ context.Context, logger *slog.Logger, _ Config, _ *traceReporter) (func(), error) {
	logger.Warn("ebpf capture unavailable on this platform; profiler is Linux-only", "goos", runtime.GOOS)
	return nil, fmt.Errorf("%w: not linux (%s)", ErrUnsupported, runtime.GOOS)
}
