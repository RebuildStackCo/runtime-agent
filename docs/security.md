...
| `inventory` | controller + node DaemonSet | host PID, `CAP_SYS_PTRACE` (see [§7.1](#71-the-go-binary-scanner-the-current-node-role)) | Above + which of your binaries are Go, their version and module path (ADR 0009), which of them serve a `/debug/pprof` endpoint and where ([§10.3](#103-pprof-endpoint-discovery-and-profile-pulling)) |
...
| Agent metrics | your monitoring → both roles, in-cluster only | Alert on the agent itself: whether it is still collecting, which of its reads are failing, what its filters excluded, what its spool evicted ([ADR 0070](adr/0070-the-agent-says-what-it-did-in-the-format-the-cluster-already-reads.md)) | **`/metrics` on the same port 9090 as the probes above, in every profile** — GET and HEAD only, Prometheus text format, and no `ServiceMonitor` or other Prometheus Operator resource is installed. Every number is one the agent already keeps and already reports in `collection_coverage` ([§11](#11-your-controls-and-what-the-agent-says-about-itself)); the two cannot disagree, because they read the same counters. Labels come from a **closed set of nine names**, and no value is ever a namespace, pod, node, workload or image name — the counts are the same aggregates the coverage payload carries. Reached by the same NetworkPolicy rule as the probes, so where a policy is enforced any pod in the cluster may scrape it; what it learns is how many objects this agent observed and how many its filters excluded, never which |
| Collection listing | you → the controller pod, **loopback only** | Find out which objects your filters admitted and which they refused — by name, with the reason ([ADR 0084](adr/0084-what-was-excluded-is-named-inside-the-cluster.md)) | **`/collection` on `127.0.0.1:9091`, controller only.** Bound to loopback, so no Service, no container port and no NetworkPolicy rule reaches it: the only way in is `kubectl port-forward`, which your own RBAC already gates on `create` of `pods/portforward` in the agent namespace. GET and HEAD only, newline-delimited JSON, and **no query is parsed** — the whole list is always answered, so an empty reply can never be mistaken for "nothing matched". This is the one place the agent names an object your filters excluded, and what it answers **never leaves the cluster**: nothing that can write a payload or open a connection can reach it, and a test on the import graph is what keeps that true. Off with one Helm value. An address that is not loopback is refused at startup rather than opened |
...
| `node_lifecycle` | Per hour: each node that joined or left the cluster, with its instance and capacity type, zone, capacity and accelerators. The size travels with the event because a node that left is gone from `node_metadata` by definition, so nothing could be joined to afterwards ([ADR 0064](adr/0064-the-node-is-described-and-its-comings-recorded.md)). An arrival is timestamped by the node object; a departure carries the instant the agent *noticed*, flagged as such. Only arrivals the agent can prove are reported — a node already present when the agent started is not an arrival | no |
...
- From node objects: `metadata.creationTimestamp`, six fields of
  `status.nodeInfo`, the instance-type, capacity-type and topology labels, size
  from `allocatable`/`capacity` — CPU, memory, ephemeral storage, pods, and
  extended resources under an allow-listed hardware-vendor prefix — the five
  allow-listed `status.conditions` **without their `message`**, and `spec.taints`,
  whose strings are bounded and dropped whole like placement values. No other
  labels, annotations, addresses or `status` fields, with the caveat in the
  `node_metadata` row above: on several managed platforms the node's *name* is
  derived from its private address.
...
- A pulled profile carries fields a captured one never had, and they go with the
  ...
  may hold anything; and the comments. The bytes that leave are rebuilt from what
  survived, so a redacted name is not left behind in a table nothing references.
...
Payloads leave only where you configure a backend address, and there is no
default. What leaves is the kinds above and nothing else, one per request, byte
for byte what the spool holds — the local sink writes exactly what the transmit
path sends, which is what makes an offline install auditable against a
connected one.

**This stage is plain HTTP.** No TLS, no certificate, no credential, and no
field naming your cluster: the backend attributes what arrives to the one source
you pointed at it, because identity is a property of a connection that does not
exist yet ([§6](#6-agent-identity-and-credential-lifecycle-planned)). Point it
only at a backend you control.

A payload leaves the spool when the backend has acknowledged it and at no other
time. Every other answer keeps it, and one that refuses this agent's identity
stops the shipping and nothing else — collection, filtering and the local spool
are untouched by anything a backend says or fails to say
([ADR 0075](adr/0075-a-payload-leaves-the-spool-only-when-the-backend-has-it.md)).
How many payloads were delivered, are waiting, or were refused and why is in
`collection_coverage` and at `/metrics` ([§11](#11-your-controls-and-what-the-agent-says-about-itself)),
so what left this cluster is countable from inside it.
...
### 10.3 pprof endpoint discovery and profile pulling
...
What it costs is bounded by construction rather than by a setting: ten seconds
per capture, one capture at a time in the whole cluster, at most ten per
five-minute round. Workloads are visited least-recently-profiled first, so
coverage is even rather than concentrated on whichever service looks expensive.

**If you already run a continuous profiler**, Go allows only one CPU profile per
process at a time, so one of the two requests loses. When it is ours, the
workload is left alone for six hours rather than competed with, and the refusal
is counted in `collection_coverage` so a report can say why that service has no
profile. There remains a narrow window — your profiler idle at the moment we
...
assembled, so the controller holds the unreduced stacks for the moments between
the read and the filter. Nothing unreduced leaves the cluster either way, and
this is the only path where the reduction is not physically upstream of the
transfer.
...
The agent tallies pods observed and pods excluded by each filter. Those counts
leave in `collection_coverage`, and the same counters are readable inside your
cluster at `/metrics` ([§5](#5-network-access)) — so filtering behavior is
