...
identity-e2e: kind-load ## Deploy controller + node DaemonSet in kind and assert a real node token cannot speak for another node, and that a token not projected into a pod is refused (ADR 0040); log goes to test/e2e/logs/
	@mkdir -p test/e2e/logs
	docker pull $(SPOOL_READER_IMAGE)
	go tool kind load docker-image $(SPOOL_READER_IMAGE) --name $(E2E_CLUSTER)
	set -o pipefail; \
	...
	E2E_AGENT_IMAGE=$(IMAGE):$(IMAGE_TAG) \
	E2E_SPOOL_READER_IMAGE=$(SPOOL_READER_IMAGE) \
	go test -tags e2e -count=1 -timeout 20m -v ./test/e2e/ -run 'TestANodeCannotSpeakForAnotherNode|TestATokenThatWasNotProjectedIntoAPodIsRefused' 2>&1 \
		| tee test/e2e/logs/identity-e2e-$$(date +%Y%m%d-%H%M%S).log
